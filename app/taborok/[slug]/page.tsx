...
import { MapPin, Calendar, Users, ArrowRight, Sparkles, Check, Shield, Clock } from "lucide-react"
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!camp) return { title: "Tabor nem talalhato" }

  return {
    title: `${camp.city} - ${camp.clubName} Tabor | Benfica Football Camp Hungary`,
    description: camp.description || `${camp.clubName} futballtabor ${camp.city} helyszinnel, ${camp.dates}. ${camp.ageRange} eves korosztalynak.`,
    openGraph: {
      title: `${camp.city} - ${camp.clubName} Tabor`,
      ...
      <section className="relative pt-36 pb-20 md:pt-44 md:pb-28 overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12 lg:px-24">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/50 text-sm">{camp.ageRange} eves korosztaly</span>
          </div>
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-6 bg-[#0a1f0a] border border-[#d4a017]/15">
                  ...
                  <span className="block text-muted-foreground text-xs mt-1">Teljes het</span>
                </div>
                <div className="text-center p-6 bg-[#0a1f0a] border border-[#d4a017]/15">
                  <Clock className="w-6 h-6 text-[#d4a017] mx-auto mb-2" />
                  <span className="block font-serif text-xl font-bold text-foreground">4 edzes</span>
                  <span className="block text-muted-foreground text-xs mt-1">Naponta</span>
                </div>
                <div className="text-center p-6 bg-[#0a1f0a] border border-[#d4a017]/15">
                  <Users className="w-6 h-6 text-[#d4a017] mx-auto mb-2" />
                  <span className="block font-serif text-xl font-bold text-foreground">{camp.totalSpots} fo</span>
                  <span className="block text-muted-foreground text-xs mt-1">Maximum letszam</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:sticky lg:top-28 lg:self-start">
              <div className="bg-[#0a1f0a] border border-[#d4a017]/20 overflow-hidden">
                <div className="bg-[#d4a017] px-6 py-4">
                  <p className="text-[#0a1f0a] text-xs font-bold uppercase tracking-wider">Early bird ar</p>
                ...
                <div className="p-6 space-y-5">
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-white/50">Helyszin</span>
                      <span className="text-white font-medium">{camp.venue}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-white/50">Idopont</span>
                      <span className="text-white font-medium">{camp.dates}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-white/50">Korosztaly</span>
                      <span className="text-white font-medium">{camp.ageRange} ev</span>
                    </div>
                    <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 justify-center text-white/30 text-xs">
                    ...
                    <span>Biztonsagos regisztracio</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="py-16 md:py-24 bg-[#0a1f0a]">
          <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-24">
            <h2 className="font-serif text-2xl md:text-3xl font-bold text-white mb-10 text-center">
              Tobbi <span className="text-[#d4a017]">taborunk</span>
            </h2>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-baseline justify-between mt-2">
                        ...
                        <span className="text-[#d4a017] text-sm font-medium group-hover:underline flex items-center gap-1">
                          Reszletek <ArrowRight className="w-3.5 h-3.5" />
                        </span>
                      </div>
                    </div>
                  </div>
