...
import { Palette, GraduationCap, Settings, CalendarDays, ArrowRight, Handshake, Phone } from "lucide-react"
...
const STEPS = [
  ...
  { num: "02", title: "Egyeztetjük az időpontot és helyszínt", desc: "Közösen kiválasztjuk a megfelelő időpontot és helyszínt a tábor számára." },
  { num: "03", title: "Elkészítjük a közös arculatot", desc: "Marketing anyagokat készítünk a te klubod arculatával és a Benfica branddel." },
  { num: "04", title: "Elindítjuk a közös tábort", desc: "Mi végezzük a teljes szervezést, te csak a játékosaidat hozod!" },
]
        ...
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 lg:px-24">
          <div className="grid sm:grid-cols-2 gap-6">
            {t.partnerProgram.benefits.map((benefit, index) => {
              ...
              return (
                <div key={benefit.title} className="group p-8 bg-white border border-border/50 hover:border-[#d4a017]/50 hover:shadow-lg transition-all duration-300">
                  <div className="w-14 h-14 bg-[#0a1f0a] flex items-center justify-center mb-5 group-hover:bg-[#d4a017] transition-colors duration-300">
                    <Icon className="w-7 h-7 text-[#d4a017] group-hover:text-[#0a1f0a] transition-colors" strokeWidth={1.5} />
                  </div>
                  <h3 className="font-serif text-xl font-bold text-foreground mb-2 group-hover:text-[#d4a017] transition-colors">{benefit.title}</h3>
